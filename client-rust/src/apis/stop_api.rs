...
/// struct for typed errors of method [`get_stop`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetStopError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`update_stop`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum UpdateStopError {
    UnknownValue(serde_json::Value),
}
...
pub async fn get_stop(configuration: &configuration::Configuration, version: f64, id: i64) -> Result<models::Stop, Error<GetStopError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetStopError> = serde_json::from_str(&content).ok();
...
pub async fn update_stop(configuration: &configuration::Configuration, version: f64, id: i64, body: Option<models::Stop>) -> Result<models::Stop, Error<UpdateStopError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<UpdateStopError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
