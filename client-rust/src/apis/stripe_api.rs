...
/// struct for typed errors of method [`create_stripe_checkout_session`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CreateStripeCheckoutSessionError {
...
pub async fn create_stripe_checkout_session(configuration: &configuration::Configuration, app_key: &str, stripe_parameters: &str) -> Result<models::SirqulResponse, Error<CreateStripeCheckoutSessionError>> {
    ...
    req_builder = req_builder.query(&[("stripeParameters", &p_query_stripe_parameters.to_string())]);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<CreateStripeCheckoutSessionError> = serde_json::from_str(&content).ok();
