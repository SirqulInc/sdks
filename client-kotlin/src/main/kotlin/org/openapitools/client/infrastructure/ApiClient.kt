...
open class ApiClient(val baseUrl: String, val client: Call.Factory = defaultClient) {
    ...
    protected fun <T> updateAuthParams(requestConfig: RequestConfig<T>) {
        if (requestConfig.headers["Application-Key"].isNullOrEmpty()) {
            if (apiKey["Application-Key"] != null) {
                if (apiKeyPrefix["Application-Key"] != null) {
                    requestConfig.headers["Application-Key"] = apiKeyPrefix["Application-Key"]!! + " " + apiKey["Application-Key"]!!
                } else {
                    requestConfig.headers["Application-Key"] = apiKey["Application-Key"]!!
                }
            }
        }
        if (requestConfig.headers["Application-Rest-Key"].isNullOrEmpty()) {
            if (apiKey["Application-Rest-Key"] != null) {
                if (apiKeyPrefix["Application-Rest-Key"] != null) {
                    requestConfig.headers["Application-Rest-Key"] = apiKeyPrefix["Application-Rest-Key"]!! + " " + apiKey["Application-Rest-Key"]!!
                } else {
                    requestConfig.headers["Application-Rest-Key"] = apiKey["Application-Rest-Key"]!!
                }
            }
        }
    }
