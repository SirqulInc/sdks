(ns sirqul-io-t-platform.specs.connection-list-response
  (:require [clojure.spec.alpha :as s]
            [spec-tools.data-spec :as ds]
            [sirqul-io-t-platform.specs.name-string-value-response :refer :all]
            [sirqul-io-t-platform.specs.connection-response :refer :all]
            )
  (:import (java.io File)))


(def connection-list-response-data
  {
   (ds/opt :valid) boolean?
   (ds/opt :message) string?
   (ds/opt :version) float?
   (ds/opt :serializeNulls) boolean?
   (ds/opt :startTimeLog) int?
   (ds/opt :errorCode) string?
   (ds/opt :request) (s/coll-of name-string-value-response-spec)
   (ds/opt :connections) (s/coll-of connection-response-spec)
   (ds/opt :friendCount) int?
   (ds/opt :followingCount) int?
   (ds/opt :itemCount) int?
   (ds/opt :hasMoreResults) boolean?
   (ds/opt :returning) string?
   })

(def connection-list-response-spec
  (ds/spec
    {:name ::connection-list-response
     :spec connection-list-response-data}))
