...
(defn-spec batch-update-offer-locations-with-http-info any?
  ...
  ([data string?, ] (batch-update-offer-locations-with-http-info data nil))
  ([data string?, {:keys [deviceId accountId]} (s/map-of keyword? any?)]
   (check-required-params data)
   (call-api "/retailer/offer/location/batchUpdate" :post
             {:path-params   {}
              :header-params {}
...
(defn-spec batch-update-offer-locations sirqul-response-spec
  "Update Offer Locations
  Batch update offer locations."
  ([data string?, ] (batch-update-offer-locations data nil))
  ([data string?, optional-params any?]
   (let [res (:data (batch-update-offer-locations-with-http-info data optional-params))]
...
(defn-spec create-offer-with-http-info any?
  ...
  ([includeOfferLocations boolean?, title string?, barcodeType string?, noExpiration boolean?, availableLimit int?, availableLimitPerUser int?, addedLimit int?, viewLimit int?, maxPrints int?, ticketPrice int?, fullPrice float?, discountPrice float?, offerType string?, specialOfferType string?, offerVisibility string?, active boolean?, ] (create-offer-with-http-info includeOfferLocations title barcodeType noExpiration availableLimit availableLimitPerUser addedLimit viewLimit maxPrints ticketPrice fullPrice discountPrice offerType specialOfferType offerVisibility active nil))
  ([includeOfferLocations boolean?, title string?, barcodeType string?, noExpiration boolean?, availableLimit int?, availableLimitPerUser int?, addedLimit int?, viewLimit int?, maxPrints int?, ticketPrice int?, fullPrice float?, discountPrice float?, offerType string?, specialOfferType string?, offerVisibility string?, active boolean?, {:keys [deviceId accountId tags parentOfferId retailerLocationIds offerLocations subTitle details subDetails finePrint barcodeEntry externalRedeemOptions externalUrl externalId ticketsRewardType ticketsReward activated expires ticketPriceType showRemaining showRedeemed replaced featured categoryIds filterIds barcodeAssetId imageAssetId imageAssetId1 imageAssetId2 imageAssetId3 imageAssetId4 imageAssetId5 publisher redeemableStart redeemableEnd brand productType conditionType isbn asin catalogNumbers department features minimumPrice width height depth weight unit studio parentalRating publishDate availabilityDate sizeId listingId mediaType duration author releaseDate collectionIds rebootTimeHour rebootTimeMinute idleTimeoutInSecond serialNumber udid deviceType devicePower deviceInterference availability availabilitySummary]} (s/map-of keyword? any?)]
   (check-required-params includeOfferLocations title barcodeType noExpiration availableLimit availableLimitPerUser addedLimit viewLimit maxPrints ticketPrice fullPrice discountPrice offerType specialOfferType offerVisibility active)
   (call-api "/retailer/offer/create" :post
             {:path-params   {}
              :header-params {}
...
(defn-spec create-offer retailer-offer-response-spec
  "Create Offer
  Create an offer and assign it to the provided retailer locations."
  ([includeOfferLocations boolean?, title string?, barcodeType string?, noExpiration boolean?, availableLimit int?, availableLimitPerUser int?, addedLimit int?, viewLimit int?, maxPrints int?, ticketPrice int?, fullPrice float?, discountPrice float?, offerType string?, specialOfferType string?, offerVisibility string?, active boolean?, ] (create-offer includeOfferLocations title barcodeType noExpiration availableLimit availableLimitPerUser addedLimit viewLimit maxPrints ticketPrice fullPrice discountPrice offerType specialOfferType offerVisibility active nil))
  ([includeOfferLocations boolean?, title string?, barcodeType string?, noExpiration boolean?, availableLimit int?, availableLimitPerUser int?, addedLimit int?, viewLimit int?, maxPrints int?, ticketPrice int?, fullPrice float?, discountPrice float?, offerType string?, specialOfferType string?, offerVisibility string?, active boolean?, optional-params any?]
   (let [res (:data (create-offer-with-http-info includeOfferLocations title barcodeType noExpiration availableLimit availableLimitPerUser addedLimit viewLimit maxPrints ticketPrice fullPrice discountPrice offerType specialOfferType offerVisibility active optional-params))]
     (if (:decode-models *api-context*)
...
(defn-spec delete-offer-with-http-info any?
  ...
  ([offerId int?, ] (delete-offer-with-http-info offerId nil))
  ([offerId int?, {:keys [deviceId accountId]} (s/map-of keyword? any?)]
   (check-required-params offerId)
   (call-api "/retailer/offer/delete" :post
             {:path-params   {}
              :header-params {}
...
(defn-spec delete-offer sirqul-response-spec
  "Delete Offer
  Set the deleted timestamp to current time. This effectively deletes the offer since all queries should ignore any records with a deleted time stamp."
  ([offerId int?, ] (delete-offer offerId nil))
  ([offerId int?, optional-params any?]
   (let [res (:data (delete-offer-with-http-info offerId optional-params))]
     (if (:decode-models *api-context*)
        (st/decode sirqul-response-spec res st/string-transformer)
        res))))
...
(defn-spec delete-offer-location-with-http-info any?
  ...
  ([offerLocationId int?, ] (delete-offer-location-with-http-info offerLocationId nil))
  ([offerLocationId int?, {:keys [deviceId accountId]} (s/map-of keyword? any?)]
   (check-required-params offerLocationId)
   (call-api "/retailer/offer/location/delete" :post
             {:path-params   {}
              :header-params {}
...
(defn-spec delete-offer-location sirqul-response-spec
  "Delete Offer Location
  Set the deleted timestamp to current time. This effectively deletes the offer location since all queries should ignore any records with a deleted time stamp."
  ([offerLocationId int?, ] (delete-offer-location offerLocationId nil))
  ([offerLocationId int?, optional-params any?]
   (let [res (:data (delete-offer-location-with-http-info offerLocationId optional-params))]
     (if (:decode-models *api-context*)
        (st/decode sirqul-response-spec res st/string-transformer)
        res))))
...
(defn-spec get-offer-with-http-info any?
  ...
  ([offerId int?, includeOfferLocations boolean?, ] (get-offer-with-http-info offerId includeOfferLocations nil))
  ([offerId int?, includeOfferLocations boolean?, {:keys [deviceId accountId]} (s/map-of keyword? any?)]
   (check-required-params offerId includeOfferLocations)
   (call-api "/retailer/offer/get" :get
             {:path-params   {}
              :header-params {}
...
(defn-spec get-offer retailer-offer-response-spec
  "Get Offer
  Gets the details of an offer that the user has access to."
  ([offerId int?, includeOfferLocations boolean?, ] (get-offer offerId includeOfferLocations nil))
  ([offerId int?, includeOfferLocations boolean?, optional-params any?]
   (let [res (:data (get-offer-with-http-info offerId includeOfferLocations optional-params))]
     (if (:decode-models *api-context*)
        (st/decode retailer-offer-response-spec res st/string-transformer)
        res))))
...
(defn-spec get-offer-details-with-http-info any?
  ...
  ([] (get-offer-details-with-http-info nil))
  ([{:keys [deviceId accountId offerId offerLocationId distance latitude longitude includeOfferLocations includeRetailerLocations includeChildOffers]} (s/map-of keyword? any?)]
   (call-api "/offer/get" :get
             {:path-params   {}
              :header-params {}
...
(defn-spec get-offer-details offer-response-spec
  "Get Offer
  Gets offer or offer location details as a consumer.  Will check if it is a favorite if the deviceId/accountId is provided.  If the offerId is provided it will look up the main offer and ignore the the offerLocationId. If no offerId is provided then an offerLocationId must be specified."
  ([] (get-offer-details nil))
  ([optional-params any?]
   (let [res (:data (get-offer-details-with-http-info optional-params))]
     (if (:decode-models *api-context*)
...
(defn-spec get-offer-list-counts-with-http-info any?
  ...
  ([latitude float?, longitude float?, ] (get-offer-list-counts-with-http-info latitude longitude nil))
  ([latitude float?, longitude float?, {:keys [searchRange distanceUnit]} (s/map-of keyword? any?)]
   (check-required-params latitude longitude)
   (call-api "/offer/lists/count" :get
             {:path-params   {}
              :header-params {}
...
(defn-spec get-offer-list-counts list-count-response-spec
  "Get Offers (Counts)
  Gets the offer list counts."
  ([latitude float?, longitude float?, ] (get-offer-list-counts latitude longitude nil))
  ([latitude float?, longitude float?, optional-params any?]
   (let [res (:data (get-offer-list-counts-with-http-info latitude longitude optional-params))]
     (if (:decode-models *api-context*)
...
(defn-spec get-offer-location-with-http-info any?
  ...
  ([] (get-offer-location-with-http-info nil))
  ([{:keys [offerLocationId udid]} (s/map-of keyword? any?)]
   (call-api "/offer/location/get" :get
             {:path-params   {}
              :header-params {}
...
(defn-spec get-offer-location offer-short-response-spec
  "Get Offer Location
  Gets the offer location by offer location id or udid (of a device)"
  ([] (get-offer-location nil))
  ([optional-params any?]
   (let [res (:data (get-offer-location-with-http-info optional-params))]
     (if (:decode-models *api-context*)
...
(defn-spec get-offer-locations-for-retailers-with-http-info any?
  ...
  ([sortField string?, descending boolean?, start int?, limit int?, activeOnly boolean?, includeRetailerLocation boolean?, ] (get-offer-locations-for-retailers-with-http-info sortField descending start limit activeOnly includeRetailerLocation nil))
  ([sortField string?, descending boolean?, start int?, limit int?, activeOnly boolean?, includeRetailerLocation boolean?, {:keys [deviceId accountId keyword retailerId retailerLocationId offerType specialOfferType barcodeType barcodeEntry isbn asin deviceStatus needsNotificationSent lastNotificationSent]} (s/map-of keyword? any?)]
   (check-required-params sortField descending start limit activeOnly includeRetailerLocation)
   (call-api "/retailer/offer/location/search" :get
             {:path-params   {}
              :header-params {}
...
(defn-spec get-offer-locations-for-retailers (s/coll-of offer-short-response-spec)
  "Search Offer Locations
  Searches on offer locations, which are records that represent an offer that has been assigned to a retailer location. If an offer does not have any locations assigned, then it will NOT be returned."
  ([sortField string?, descending boolean?, start int?, limit int?, activeOnly boolean?, includeRetailerLocation boolean?, ] (get-offer-locations-for-retailers sortField descending start limit activeOnly includeRetailerLocation nil))
  ([sortField string?, descending boolean?, start int?, limit int?, activeOnly boolean?, includeRetailerLocation boolean?, optional-params any?]
   (let [res (:data (get-offer-locations-for-retailers-with-http-info sortField descending start limit activeOnly includeRetailerLocation optional-params))]
     (if (:decode-models *api-context*)
...
(defn-spec get-offers-for-retailers-with-http-info any?
  ...
  ([offerVisibility string?, sortField string?, descending boolean?, start int?, limit int?, availableOnly boolean?, activeOnly boolean?, includeCategories boolean?, includeFilters boolean?, includeOfferLocations boolean?, ] (get-offers-for-retailers-with-http-info offerVisibility sortField descending start limit availableOnly activeOnly includeCategories includeFilters includeOfferLocations nil))
  ([offerVisibility string?, sortField string?, descending boolean?, start int?, limit int?, availableOnly boolean?, activeOnly boolean?, includeCategories boolean?, includeFilters boolean?, includeOfferLocations boolean?, {:keys [deviceId accountId categoryIds filterIds q keyword retailerId retailerLocationId couponType offerType offerTypes specialOfferType _i _l barcodeType barcodeEntry isbn asin deviceStatus needsNotificationSent lastNotificationSent]} (s/map-of keyword? any?)]
   (check-required-params offerVisibility sortField descending start limit availableOnly activeOnly includeCategories includeFilters includeOfferLocations)
   (call-api "/retailer/offer/search" :get
             {:path-params   {}
              :header-params {}
...
(defn-spec get-offers-for-retailers (s/coll-of offer-response-spec)
  "Search Offers
  Searches on offers that the account has access to."
  ([offerVisibility string?, sortField string?, descending boolean?, start int?, limit int?, availableOnly boolean?, activeOnly boolean?, includeCategories boolean?, includeFilters boolean?, includeOfferLocations boolean?, ] (get-offers-for-retailers offerVisibility sortField descending start limit availableOnly activeOnly includeCategories includeFilters includeOfferLocations nil))
  ([offerVisibility string?, sortField string?, descending boolean?, start int?, limit int?, availableOnly boolean?, activeOnly boolean?, includeCategories boolean?, includeFilters boolean?, includeOfferLocations boolean?, optional-params any?]
   (let [res (:data (get-offers-for-retailers-with-http-info offerVisibility sortField descending start limit availableOnly activeOnly includeCategories includeFilters includeOfferLocations optional-params))]
     (if (:decode-models *api-context*)
...
(defn-spec redeem-offer-transaction-with-http-info any?
  ...
  Redeems an offer."
  ([offerTransactionId int?, status int?, ] (redeem-offer-transaction-with-http-info offerTransactionId status nil))
  ([offerTransactionId int?, status int?, {:keys [deviceId accountId offerLocationId]} (s/map-of keyword? any?)]
   (check-required-params offerTransactionId status)
   (call-api "/retailer/offer/transaction/update" :post
             {:path-params   {}
              :header-params {}
...
(defn-spec redeem-offer-transaction sirqul-response-spec
  "Update Offer Transaction
  Redeems an offer."
  ([offerTransactionId int?, status int?, ] (redeem-offer-transaction offerTransactionId status nil))
  ([offerTransactionId int?, status int?, optional-params any?]
   (let [res (:data (redeem-offer-transaction-with-http-info offerTransactionId status optional-params))]
     (if (:decode-models *api-context*)
        (st/decode sirqul-response-spec res st/string-transformer)
        res))))
...
(defn-spec search-offer-transactions-for-retailers-with-http-info any?
  ...
  ([sortField string?, descending boolean?, start int?, limit int?, activeOnly boolean?, ] (search-offer-transactions-for-retailers-with-http-info sortField descending start limit activeOnly nil))
  ([sortField string?, descending boolean?, start int?, limit int?, activeOnly boolean?, {:keys [deviceId accountId q keyword retailerId retailerLocationId offerId offerLocationId redeemed reservationsOnly couponType offerType specialOfferType customerAccountIds categoryIds redeemableStartDate redeemableEndDate _i _l]} (s/map-of keyword? any?)]
   ...
   (call-api "/retailer/offer/transaction/search" :get
             {:path-params   {}
              :header-params {}
...
(defn-spec search-offer-transactions-for-retailers (s/coll-of offer-transaction-response-spec)
  "Search Offer Transactions
  Searches on offer transactions for offers that the account has access to."
  ([sortField string?, descending boolean?, start int?, limit int?, activeOnly boolean?, ] (search-offer-transactions-for-retailers sortField descending start limit activeOnly nil))
  ([sortField string?, descending boolean?, start int?, limit int?, activeOnly boolean?, optional-params any?]
   (let [res (:data (search-offer-transactions-for-retailers-with-http-info sortField descending start limit activeOnly optional-params))]
     (if (:decode-models *api-context*)
...
(defn-spec search-offers-for-consumer-with-http-info any?
  ...
  ([latitude float?, longitude float?, recommendationType string?, locationId int?, start int?, limit int?, maxRecommendations int?, distanceUnit string?, ] (search-offers-for-consumer-with-http-info latitude longitude recommendationType locationId start limit maxRecommendations distanceUnit nil))
  ([latitude float?, longitude float?, recommendationType string?, locationId int?, start int?, limit int?, maxRecommendations int?, distanceUnit string?, {:keys [appKey deviceId accountId searchRange tags supportedPostalCodes keyword categories filters offerTypes type sortField recommendOfferIds retailerLocationIds offerId includeMission includeCategories includeFilters includeExpired includeFavorite closestOfferOnly searchExpression groupBy]} (s/map-of keyword? any?)]
   (check-required-params latitude longitude recommendationType locationId start limit maxRecommendations distanceUnit)
   (call-api "/offer/lists" :get
             {:path-params   {}
              :header-params {}
...
(defn-spec search-offers-for-consumer offer-list-response-spec
  "Search Offers
  Searches for offers as a consumer."
  ([latitude float?, longitude float?, recommendationType string?, locationId int?, start int?, limit int?, maxRecommendations int?, distanceUnit string?, ] (search-offers-for-consumer latitude longitude recommendationType locationId start limit maxRecommendations distanceUnit nil))
  ([latitude float?, longitude float?, recommendationType string?, locationId int?, start int?, limit int?, maxRecommendations int?, distanceUnit string?, optional-params any?]
   (let [res (:data (search-offers-for-consumer-with-http-info latitude longitude recommendationType locationId start limit maxRecommendations distanceUnit optional-params))]
     (if (:decode-models *api-context*)
...
(defn-spec top-offer-transactions-with-http-info any?
  ...
  ([] (top-offer-transactions-with-http-info nil))
  ([{:keys [start limit]} (s/map-of keyword? any?)]
   (call-api "/offer/top" :get
             {:path-params   {}
              :header-params {}
...
(defn-spec top-offer-transactions offer-list-response-spec
  "Get Offers (Top)
  Gets the top active offers."
  ([] (top-offer-transactions nil))
  ([optional-params any?]
   (let [res (:data (top-offer-transactions-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode offer-list-response-spec res st/string-transformer)
        res))))
...
(defn-spec update-offer retailer-offer-response-spec
  "Update Offer
  Update an offer, must provide a current list of retailer locations or the current offer locations will be marked as deleted."
  ([offerId int?, includeOfferLocations boolean?, ] (update-offer offerId includeOfferLocations nil))
  ([offerId int?, includeOfferLocations boolean?, optional-params any?]
   (let [res (:data (update-offer-with-http-info offerId includeOfferLocations optional-params))]
     (if (:decode-models *api-context*)
        (st/decode retailer-offer-response-spec res st/string-transformer)
        res))))
...
(defn-spec update-offer-status-with-http-info any?
  ...
  ([offerIds string?, active boolean?, ] (update-offer-status-with-http-info offerIds active nil))
  ([offerIds string?, active boolean?, {:keys [deviceId accountId]} (s/map-of keyword? any?)]
   (check-required-params offerIds active)
   (call-api "/retailer/offer/status" :post
             {:path-params   {}
              :header-params {}
...
(defn-spec update-offer-status sirqul-response-spec
  "Activate Offer
  Sets the activated date on offers. This will make offers visible for consumers."
  ([offerIds string?, active boolean?, ] (update-offer-status offerIds active nil))
  ([offerIds string?, active boolean?, optional-params any?]
   (let [res (:data (update-offer-status-with-http-info offerIds active optional-params))]
     (if (:decode-models *api-context*)
        (st/decode sirqul-response-spec res st/string-transformer)
        res))))
