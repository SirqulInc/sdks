...
sub update_params_for_auth {
    ...
    foreach my $auth (@$auth_settings) {
        ...
        elsif ($auth eq 'appKey') {
            my $api_key = $self->get_api_key_with_prefix('Application-Key');
            if ($api_key) {
                $header_params->{'Application-Key'} = $api_key;
            }
        }
        elsif ($auth eq 'restKey') {
            my $api_key = $self->get_api_key_with_prefix('Application-Rest-Key');
            if ($api_key) {
                $header_params->{'Application-Rest-Key'} = $api_key;
            }
        }
        else {
