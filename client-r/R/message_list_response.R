#' Create a new MessageListResponse
#'
#' @description
...
MessageListResponse <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new MessageListResponse class.
    #'
    #' @param start start
    #' @param limit limit
    ...
    #'
    #' @return MessageListResponse as a base R list.
    #' @examples
    #' # convert array of MessageListResponse (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert MessageListResponse to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      MessageListResponseObject <- list()
      if (!is.null(self$`start`)) {
        MessageListResponseObject[["start"]] <-
          self$`start`
      }
      if (!is.null(self$`limit`)) {
        MessageListResponseObject[["limit"]] <-
          self$`limit`
      }
      if (!is.null(self$`countTotal`)) {
        MessageListResponseObject[["countTotal"]] <-
          self$`countTotal`
      }
      if (!is.null(self$`items`)) {
        MessageListResponseObject[["items"]] <-
          self$extractSimpleType(self$`items`)
      }
      if (!is.null(self$`count`)) {
        MessageListResponseObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`hasMoreResults`)) {
        MessageListResponseObject[["hasMoreResults"]] <-
          self$`hasMoreResults`
      }
    ...
    #' @description
    #' Deserialize JSON string into an instance of MessageListResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of MessageListResponse
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`items`)) {
        self$`items` <- ApiClient$new()$deserializeObj(this_object$`items`, "array[MessageResponse]", loadNamespace("openapi"))
      }
    ...
    #' @return MessageListResponse in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to MessageListResponse and throw an exception if invalid
    #'
    ...
    #'
    #' @return String representation of MessageListResponse
    toString = function() {
      self$toJSONString()
    },
...
# MessageListResponse$unlock()
#
## Below is an example to define the print function
# MessageListResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# MessageListResponse$lock()
