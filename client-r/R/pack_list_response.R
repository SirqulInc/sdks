...
PackListResponse <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new PackListResponse class.
    #'
    #' @param start start
    #' @param limit limit
    ...
    #'
    #' @return PackListResponse as a base R list.
    #' @examples
    #' # convert array of PackListResponse (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PackListResponse to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      PackListResponseObject <- list()
      if (!is.null(self$`start`)) {
        PackListResponseObject[["start"]] <-
          self$`start`
      }
      if (!is.null(self$`limit`)) {
        PackListResponseObject[["limit"]] <-
          self$`limit`
      }
      if (!is.null(self$`countTotal`)) {
        PackListResponseObject[["countTotal"]] <-
          self$`countTotal`
      }
      if (!is.null(self$`items`)) {
        PackListResponseObject[["items"]] <-
          self$extractSimpleType(self$`items`)
      }
      if (!is.null(self$`count`)) {
        PackListResponseObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`hasMoreResults`)) {
        PackListResponseObject[["hasMoreResults"]] <-
          self$`hasMoreResults`
      }
      return(PackListResponseObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of PackListResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of PackListResponse
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`items`)) {
        self$`items` <- ApiClient$new()$deserializeObj(this_object$`items`, "array[PackResponse]", loadNamespace("openapi"))
      }
    ...
    #' @return PackListResponse in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to PackListResponse and throw an exception if invalid
    #'
    ...
    #'
    #' @return String representation of PackListResponse
    toString = function() {
      self$toJSONString()
    },
...
# PackListResponse$unlock()
#
## Below is an example to define the print function
# PackListResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PackListResponse$lock()
