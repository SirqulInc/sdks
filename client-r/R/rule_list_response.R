...
RuleListResponse <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new RuleListResponse class.
    #'
    #' @param start start
    #' @param limit limit
    ...
    #'
    #' @return RuleListResponse as a base R list.
    #' @examples
    #' # convert array of RuleListResponse (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert RuleListResponse to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      RuleListResponseObject <- list()
      if (!is.null(self$`start`)) {
        RuleListResponseObject[["start"]] <-
          self$`start`
      }
      if (!is.null(self$`limit`)) {
        RuleListResponseObject[["limit"]] <-
          self$`limit`
      }
      if (!is.null(self$`countTotal`)) {
        RuleListResponseObject[["countTotal"]] <-
          self$`countTotal`
      }
      if (!is.null(self$`items`)) {
        RuleListResponseObject[["items"]] <-
          self$extractSimpleType(self$`items`)
      }
      if (!is.null(self$`count`)) {
        RuleListResponseObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`hasMoreResults`)) {
        RuleListResponseObject[["hasMoreResults"]] <-
          self$`hasMoreResults`
      }
      return(RuleListResponseObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of RuleListResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of RuleListResponse
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`items`)) {
        self$`items` <- ApiClient$new()$deserializeObj(this_object$`items`, "array[RuleResponse]", loadNamespace("openapi"))
      }
    ...
    #' @return RuleListResponse in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to RuleListResponse and throw an exception if invalid
    #'
    ...
    #'
    #' @return String representation of RuleListResponse
    toString = function() {
      self$toJSONString()
    },
...
# RuleListResponse$unlock()
#
## Below is an example to define the print function
# RuleListResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# RuleListResponse$lock()
