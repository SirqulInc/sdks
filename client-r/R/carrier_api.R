...
#' var_active_only <- TRUE # character | Determines whether to return inactive results (Optional)
#'
#' #Search Carriers
...
#' @export
CarrierApi <- R6::R6Class(
  "CarrierApi",
  public = list(
    api_client = NULL,

    #' @description
    #' Initialize a new CarrierApi.
    #'
    ...
    #' @param active_only (optional) Determines whether to return inactive results (default value: TRUE)
    ...
    SearchCarriersWithHttpInfo = function(keyword = NULL, descending = FALSE, start = 0, limit = 20, active_only = TRUE, data_file = NULL, ..., .parse = TRUE) {
      ...
      if (!missing(`descending`) && is.null(`descending`)) {
        stop("Invalid value for `descending` when calling CarrierApi$SearchCarriers, `descending` is not nullable")
      }

      if (!missing(`start`) && is.null(`start`)) {
        stop("Invalid value for `start` when calling CarrierApi$SearchCarriers, `start` is not nullable")
      }

      if (!missing(`limit`) && is.null(`limit`)) {
        stop("Invalid value for `limit` when calling CarrierApi$SearchCarriers, `limit` is not nullable")
      }

      if (!missing(`active_only`) && is.null(`active_only`)) {
        stop("Invalid value for `active_only` when calling CarrierApi$SearchCarriers, `active_only` is not nullable")
      }
      ...
      if (local_var_resp$status_code >= 200 && local_var_resp$status_code <= 299) {
        ...
        deserialized_resp_obj <- tryCatch(
          self$api_client$DeserializeResponse(local_var_resp, "array[CellCarrierResponse]"),
          error = function(e) {
