#' Create a new ScoreListResponse
#'
#' @description
...
ScoreListResponse <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new ScoreListResponse class.
    #'
    #' @param start start
    #' @param limit limit
    ...
    #'
    #' @return ScoreListResponse as a base R list.
    #' @examples
    #' # convert array of ScoreListResponse (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert ScoreListResponse to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      ScoreListResponseObject <- list()
      if (!is.null(self$`start`)) {
        ScoreListResponseObject[["start"]] <-
          self$`start`
      }
      if (!is.null(self$`limit`)) {
        ScoreListResponseObject[["limit"]] <-
          self$`limit`
      }
      if (!is.null(self$`countTotal`)) {
        ScoreListResponseObject[["countTotal"]] <-
          self$`countTotal`
      }
      if (!is.null(self$`items`)) {
        ScoreListResponseObject[["items"]] <-
          self$extractSimpleType(self$`items`)
      }
      if (!is.null(self$`count`)) {
        ScoreListResponseObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`hasMoreResults`)) {
        ScoreListResponseObject[["hasMoreResults"]] <-
          self$`hasMoreResults`
      }
      return(ScoreListResponseObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of ScoreListResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of ScoreListResponse
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`items`)) {
        self$`items` <- ApiClient$new()$deserializeObj(this_object$`items`, "array[ScoreResponse]", loadNamespace("openapi"))
      }
    ...
    #' @return ScoreListResponse in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to ScoreListResponse and throw an exception if invalid
    #'
    ...
    #'
    #' @return String representation of ScoreListResponse
    toString = function() {
      self$toJSONString()
    },
...
# ScoreListResponse$unlock()
#
## Below is an example to define the print function
# ScoreListResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# ScoreListResponse$lock()
