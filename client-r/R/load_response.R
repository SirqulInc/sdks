...
#' @docType class
#' @title LoadResponse
#' @description LoadResponse Class
...
LoadResponse <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new LoadResponse class.
    #'
    #' @param id id
    #' @param name name
    #' @param size size
    ...
    #'
    #' @return LoadResponse as a base R list.
    #' @examples
    #' # convert array of LoadResponse (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert LoadResponse to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      LoadResponseObject <- list()
      if (!is.null(self$`id`)) {
        LoadResponseObject[["id"]] <-
          self$`id`
      }
      if (!is.null(self$`name`)) {
        LoadResponseObject[["name"]] <-
          self$`name`
      }
      if (!is.null(self$`size`)) {
        LoadResponseObject[["size"]] <-
          self$`size`
      }
      if (!is.null(self$`loadingTime`)) {
        LoadResponseObject[["loadingTime"]] <-
          self$`loadingTime`
      }
      if (!is.null(self$`separatePayloads`)) {
        LoadResponseObject[["separatePayloads"]] <-
          self$`separatePayloads`
      }
      if (!is.null(self$`pickup`)) {
        LoadResponseObject[["pickup"]] <-
          self$extractSimpleType(self$`pickup`)
      }
      if (!is.null(self$`dropoff`)) {
        LoadResponseObject[["dropoff"]] <-
    ...
    #' @description
    #' Deserialize JSON string into an instance of LoadResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of LoadResponse
    ...
    #' @return LoadResponse in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to LoadResponse and throw an exception if invalid
    #'
    ...
    #'
    #' @return String representation of LoadResponse
    toString = function() {
      self$toJSONString()
    },
...
# LoadResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
