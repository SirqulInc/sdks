...
GameLevelListResponse <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new GameLevelListResponse class.
    #'
    #' @param start start
    #' @param limit limit
    ...
    #'
    #' @return GameLevelListResponse as a base R list.
    #' @examples
    #' # convert array of GameLevelListResponse (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert GameLevelListResponse to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      GameLevelListResponseObject <- list()
      if (!is.null(self$`start`)) {
        GameLevelListResponseObject[["start"]] <-
          self$`start`
      }
      if (!is.null(self$`limit`)) {
        GameLevelListResponseObject[["limit"]] <-
          self$`limit`
      }
      if (!is.null(self$`countTotal`)) {
        GameLevelListResponseObject[["countTotal"]] <-
          self$`countTotal`
      }
      if (!is.null(self$`items`)) {
        GameLevelListResponseObject[["items"]] <-
          self$extractSimpleType(self$`items`)
      }
      if (!is.null(self$`count`)) {
        GameLevelListResponseObject[["count"]] <-
          self$`count`
      }
      if (!is.null(self$`hasMoreResults`)) {
        GameLevelListResponseObject[["hasMoreResults"]] <-
          self$`hasMoreResults`
      }
      return(GameLevelListResponseObject)
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of GameLevelListResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of GameLevelListResponse
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`items`)) {
        self$`items` <- ApiClient$new()$deserializeObj(this_object$`items`, "array[GameLevelResponse]", loadNamespace("openapi"))
      }
    ...
    #' @return GameLevelListResponse in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to GameLevelListResponse and throw an exception if invalid
    #'
    ...
    #'
    #' @return String representation of GameLevelListResponse
    toString = function() {
      self$toJSONString()
    },
...
# GameLevelListResponse$unlock()
#
## Below is an example to define the print function
# GameLevelListResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# GameLevelListResponse$lock()
