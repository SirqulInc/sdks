...
// Functions for enum SORTFIELD for OfferStatusAPI_searchOfferTransactionStatuses

static char* searchOfferTransactionStatuses_SORTFIELD_ToString(sirqul_iot_platform_searchOfferTransactionStatuses_sortField_e SORTFIELD){
    char *SORTFIELDArray[] =  { "NULL", "ID", "CREATED", "UPDATED", "DELETED", "SEARCH_TAGS", "ACTIVE", "NAME", "DESCRIPTION", "CODE", "ROLE" };
    return SORTFIELDArray[SORTFIELD];
}

static sirqul_iot_platform_searchOfferTransactionStatuses_sortField_e searchOfferTransactionStatuses_SORTFIELD_FromString(char* SORTFIELD){
...
/*
// Function searchOfferTransactionStatuses_SORTFIELD_convertToJSON is not currently used,
// since conversion to JSON passes through the conversion of the model, and ToString. The function is kept for future reference.
//
static cJSON *searchOfferTransactionStatuses_SORTFIELD_convertToJSON(sirqul_iot_platform_searchOfferTransactionStatuses_sortField_e SORTFIELD) {
    cJSON *item = cJSON_CreateObject();
    if(cJSON_AddStringToObject(item, "sortField", searchOfferTransactionStatuses_SORTFIELD_ToString(SORTFIELD)) == NULL) {
        goto fail;
    }
    return item;
    fail:
...
// Function searchOfferTransactionStatuses_SORTFIELD_parseFromJSON is not currently used,
// since conversion from JSON passes through the conversion of the model, and FromString. The function is kept for future reference.
//
static sirqul_iot_platform_searchOfferTransactionStatuses_sortField_e searchOfferTransactionStatuses_SORTFIELD_parseFromJSON(cJSON* SORTFIELDJSON) {
    sirqul_iot_platform_searchOfferTransactionStatuses_sortField_e SORTFIELDVariable = 0;
    ...
    SORTFIELDVariable = searchOfferTransactionStatuses_SORTFIELD_FromString(SORTFIELDVar->valuestring);
...
OfferStatusAPI_createOfferTransactionStatus(apiClient_t *apiClient, char *name, int *code, char *deviceId, long accountId, double latitude, double longitude, char *description, char *role, int *active, char *applicationIds)
{
    ...
    if (code)
    {
        ...
        valueQuery_code = calloc(1,MAX_NUMBER_LENGTH);
        snprintf(valueQuery_code, MAX_NUMBER_LENGTH, "%d", *code);
    ...
    offer_transaction_status_response_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *OfferStatusAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = offer_transaction_status_response_parseFromJSON(OfferStatusAPIlocalVarJSON);
        cJSON_Delete(OfferStatusAPIlocalVarJSON);
...
OfferStatusAPI_deleteOfferTransactionStatus(apiClient_t *apiClient, long statusId, char *deviceId, long accountId, double latitude, double longitude)
{
    ...
    // query parameters
    char *keyQuery_statusId = NULL;
    char * valueQuery_statusId ;
    keyValuePair_t *keyPairQuery_statusId = 0;
    {
        keyQuery_statusId = strdup("statusId");
        valueQuery_statusId = calloc(1,MAX_NUMBER_LENGTH_LONG);
        snprintf(valueQuery_statusId, MAX_NUMBER_LENGTH_LONG, "%d", statusId);
        keyPairQuery_statusId = keyValuePair_create(keyQuery_statusId, valueQuery_statusId);
        list_addElement(localVarQueryParameters,keyPairQuery_statusId);
    }
    list_addElement(localVarHeaderType,"*/*"); //produces
    apiClient_invoke(apiClient,
                    localVarPath,
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *OfferStatusAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = sirqul_response_parseFromJSON(OfferStatusAPIlocalVarJSON);
        cJSON_Delete(OfferStatusAPIlocalVarJSON);
        if(elementToReturn == NULL) {
            // return 0;
        }
    ...
    if(keyQuery_statusId){
        free(keyQuery_statusId);
        keyQuery_statusId = NULL;
    }
    if(keyPairQuery_statusId){
        keyValuePair_free(keyPairQuery_statusId);
        keyPairQuery_statusId = NULL;
    }
    return elementToReturn;
end:
    free(localVarPath);
...
OfferStatusAPI_searchOfferTransactionStatuses(apiClient_t *apiClient, char *deviceId, long accountId, double latitude, double longitude, char *keyword, char *role, char *appKey, sirqul_iot_platform_searchOfferTransactionStatuses_sortField_e sortField, int *descending, int *start, int *limit, int *includeInactive)
{
    ...
    sirqul_iot_platform_searchOfferTransactionStatuses_sortField_e valueQuery_sortField ;
    keyValuePair_t *keyPairQuery_sortField = 0;
    if (sortField)
    {
        keyQuery_sortField = strdup("sortField");
        valueQuery_sortField = (sortField);
        keyPairQuery_sortField = keyValuePair_create(keyQuery_sortField, strdup(searchOfferTransactionStatuses_SORTFIELD_ToString(
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *OfferStatusAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        if(!cJSON_IsArray(OfferStatusAPIlocalVarJSON)) {
            return 0;//nonprimitive container
        }
        elementToReturn = list_createList();
        cJSON *VarJSON;
        cJSON_ArrayForEach(VarJSON, OfferStatusAPIlocalVarJSON)
        {
