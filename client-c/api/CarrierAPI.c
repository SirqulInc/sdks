...
CarrierAPI_searchCarriers(apiClient_t *apiClient, double version, char *keyword, int *descending, int *start, int *limit, int *activeOnly)
{
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *CarrierAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        if(!cJSON_IsArray(CarrierAPIlocalVarJSON)) {
            return 0;//nonprimitive container
        }
        elementToReturn = list_createList();
        cJSON *VarJSON;
        cJSON_ArrayForEach(VarJSON, CarrierAPIlocalVarJSON)
        {
        ...
        cJSON_Delete( CarrierAPIlocalVarJSON);
