...
VehicleAPI_createVehicle(apiClient_t *apiClient, double version, char *vehicle, vehicle_t *body)
{
    ...
    // query parameters
    char *keyQuery_vehicle = NULL;
    char * valueQuery_vehicle = NULL;
    keyValuePair_t *keyPairQuery_vehicle = 0;
    if (vehicle)
    {
        keyQuery_vehicle = strdup("vehicle");
        valueQuery_vehicle = strdup((vehicle));
        keyPairQuery_vehicle = keyValuePair_create(keyQuery_vehicle, valueQuery_vehicle);
        list_addElement(localVarQueryParameters,keyPairQuery_vehicle);
    }
    ...
    if (body != NULL)
    {
        ...
        localVarSingleItemJSON_body = vehicle_convertToJSON(body);
    ...
    vehicle_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *VehicleAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = vehicle_parseFromJSON(VehicleAPIlocalVarJSON);
        cJSON_Delete(VehicleAPIlocalVarJSON);
    ...
    if(keyQuery_vehicle){
        free(keyQuery_vehicle);
        keyQuery_vehicle = NULL;
    }
    if(valueQuery_vehicle){
        free(valueQuery_vehicle);
        valueQuery_vehicle = NULL;
    }
    if(keyPairQuery_vehicle){
        keyValuePair_free(keyPairQuery_vehicle);
        keyPairQuery_vehicle = NULL;
    }
...
VehicleAPI_searchVehicle(apiClient_t *apiClient, double version, long hubId, char *sortField, int *descending, int *start, int *limit, int *activeOnly, char *keyword)
{
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *VehicleAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        if(!cJSON_IsArray(VehicleAPIlocalVarJSON)) {
            return 0;//nonprimitive container
        }
        elementToReturn = list_createList();
        cJSON *VarJSON;
        cJSON_ArrayForEach(VarJSON, VehicleAPIlocalVarJSON)
        {
