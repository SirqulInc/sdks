...
OptimizeAPI_getOptimizationResult(apiClient_t *apiClient, double version, char *batchID, int *start, int *limit)
{
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *OptimizeAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        if(!cJSON_IsArray(OptimizeAPIlocalVarJSON)) {
            return 0;//nonprimitive container
        }
        elementToReturn = list_createList();
        cJSON *VarJSON;
        cJSON_ArrayForEach(VarJSON, OptimizeAPIlocalVarJSON)
        {
        ...
        cJSON_Delete( OptimizeAPIlocalVarJSON);
...
OptimizeAPI_requestOptimization(apiClient_t *apiClient, double version, orders_t *body)
{
    ...
    if (body != NULL)
    {
        ...
        localVarSingleItemJSON_body = orders_convertToJSON(body);
    ...
    import_statuses_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *OptimizeAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = import_statuses_parseFromJSON(OptimizeAPIlocalVarJSON);
        cJSON_Delete(OptimizeAPIlocalVarJSON);
