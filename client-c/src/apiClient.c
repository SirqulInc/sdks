...
apiClient_t *apiClient_create() {
    ...
    apiClient->basePath = strdup("https://dev.sirqul.com/api/3.18");
    ...
    apiClient->apiKeys_appKey = NULL;
    apiClient->apiKeys_restKey = NULL;
...
, list_t *apiKeys_appKey
, list_t *apiKeys_restKey
) {
    apiClient_t *apiClient = malloc(sizeof(apiClient_t));
    if(basePath){
    ...
    apiClient->progress_data = NULL;
    apiClient->response_code = 0;
    if(apiKeys_appKey!= NULL) {
        apiClient->apiKeys_appKey = list_createList();
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiKeys_appKey) {
            keyValuePair_t *pair = listEntry->data;
            keyValuePair_t *pairDup = keyValuePair_create(strdup(pair->key), strdup(pair->value));
            list_addElement(apiClient->apiKeys_appKey, pairDup);
        }
    }else{
        apiClient->apiKeys_appKey = NULL;
    }
    if(apiKeys_restKey!= NULL) {
        apiClient->apiKeys_restKey = list_createList();
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiKeys_restKey) {
            keyValuePair_t *pair = listEntry->data;
            keyValuePair_t *pairDup = keyValuePair_create(strdup(pair->key), strdup(pair->value));
            list_addElement(apiClient->apiKeys_restKey, pairDup);
        }
    }else{
        apiClient->apiKeys_restKey = NULL;
    }
...
void apiClient_free(apiClient_t *apiClient) {
    ...
    apiClient->progress_func = NULL;
    apiClient->progress_data = NULL;
    if(apiClient->apiKeys_appKey) {
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiClient->apiKeys_appKey) {
            keyValuePair_t *pair = listEntry->data;
            if(pair->key){
                free(pair->key);
            }
        ...
        list_freeList(apiClient->apiKeys_appKey);
    }
    if(apiClient->apiKeys_restKey) {
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiClient->apiKeys_restKey) {
            keyValuePair_t *pair = listEntry->data;
            if(pair->key){
                free(pair->key);
            }
        ...
        list_freeList(apiClient->apiKeys_restKey);
    }
...
void apiClient_invoke(apiClient_t    *apiClient,
    ...
    if(handle) {
        ...
        if (apiClient->apiKeys_appKey != NULL)
        {
        list_ForEach(listEntry, apiClient->apiKeys_appKey) {
        ...
        // this would only be generated for apiKey authentication
        if (apiClient->apiKeys_restKey != NULL)
        {
        list_ForEach(listEntry, apiClient->apiKeys_restKey) {
        keyValuePair_t *apiKey = listEntry->data;
        if((apiKey->key != NULL) &&
