/*
 * score_list_response.h
 *
 * 
 */

#ifndef _score_list_response_H_
#define _score_list_response_H_

#include <string.h>
#include "../external/cJSON.h"
#include "../include/list.h"
#include "../include/keyValuePair.h"
#include "../include/binary.h"

typedef struct score_list_response_t score_list_response_t;

#include "score_response.h"



typedef struct score_list_response_t {
    int start; //numeric
    int limit; //numeric
    long count_total; //numeric
    list_t *items; //nonprimitive container
    int count; //numeric
    int has_more_results; //boolean

    int _library_owned; // Is the library responsible for freeing this object?
} score_list_response_t;

__attribute__((deprecated)) score_list_response_t *score_list_response_create(
    int start,
    int limit,
    long count_total,
    list_t *items,
    int count,
    int has_more_results
);

void score_list_response_free(score_list_response_t *score_list_response);

score_list_response_t *score_list_response_parseFromJSON(cJSON *score_list_responseJSON);

cJSON *score_list_response_convertToJSON(score_list_response_t *score_list_response);

#endif /* _score_list_response_H_ */

