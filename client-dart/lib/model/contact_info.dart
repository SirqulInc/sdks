...
class ContactInfo {
  ...
  @override
  String toString() => 'ContactInfo[homePhone=$homePhone, cellPhone=$cellPhone, cellCarrier=$cellCarrier, businessPhone=$businessPhone, businessPhoneExt=$businessPhoneExt, faxNumber=$faxNumber, timeZone=$timeZone, utcOffset=$utcOffset, code501c3=$code501c3, emailAddress=$emailAddress, address=$address, web=$web]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ContactInfo[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ContactInfo[$key]" has a null value in JSON.');
        });
        return true;
      }());
