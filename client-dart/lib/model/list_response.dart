...
class ListResponse {
  ...
  @override
  String toString() => 'ListResponse[valid=$valid, message=$message, version=$version, serializeNulls=$serializeNulls, startTimeLog=$startTimeLog, errorCode=$errorCode, request=$request, items=$items, count=$count, returning=$returning]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ListResponse[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ListResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ListResponse> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ListResponse>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ListResponse.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ListResponse> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = ListResponse.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<ListResponse>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ListResponse>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ListResponse.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
