...
class ConnectionListResponse {
  ...
  @override
  String toString() => 'ConnectionListResponse[valid=$valid, message=$message, version=$version, serializeNulls=$serializeNulls, startTimeLog=$startTimeLog, errorCode=$errorCode, request=$request, connections=$connections, friendCount=$friendCount, followingCount=$followingCount, itemCount=$itemCount, hasMoreResults=$hasMoreResults, returning=$returning]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ConnectionListResponse[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ConnectionListResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());
