...
module OpenapiClient
  class AlbumFullResponse < ApiModelBase
    attr_accessor :valid
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::AlbumFullResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::AlbumFullResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'has_disliked')
        self.has_disliked = attributes[:'has_disliked']
      end
    ...
    def hash
      [valid, message, version, serialize_nulls, start_time_log, error_code, request, album_id, album_type, owner_id, owner, title, description, cover_asset, tags, visibility, start_date, end_date, asset_count, like_count, dislike_count, comments_count, user_count, notifications, latitude, longitude, location_description, distance, date_created, date_updated, cell_phone, full_address, approval_status, favorite_count, liked, favorite, favorite_id, anonymous, flag_count, viewed_count, clicked_count, shared_count, assets, comments, likes, connection_groups, connections, has_liked, has_disliked, user_permissions, public_permissions, returning].hash
    end
